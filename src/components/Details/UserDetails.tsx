...
const UserDetails: React.FC = () => {
  ...
  if (loading) return <TableLoader columns={1}></TableLoader>;
  if (!user)
    return (
      <p className="text-center text-red-500">
        ⚠️ User not found or API error!
      </p>
    );
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="bg-white shadow-lg rounded-lg p-6 flex items-center space-x-4">
        {loading ? (
          <TableLoader columns={2}></TableLoader>
        ) : user.image ? (
          <Image
            src={user.image}
            alt={user.name}
      ...
      {user.BusinessProfile ? (
        <div className="mt-6 bg-white shadow-lg rounded-lg p-6">
          ...
          <div key={user.BusinessProfile.id} className="border-b pb-3 mb-3">
            <Image
              src={user.BusinessProfile?.image}
              alt="profile images"
              height={150}
              width={150}
            ...
            <h4 className="text-md font-medium">
              {user.BusinessProfile?.businessName || "No Name"}
            </h4>
            <p className="text-gray-500">
              {user.BusinessProfile?.businessAddress || "No Address"}
            </p>
            <p className="text-sm text-gray-600">
              Open: {user.BusinessProfile?.startDay} -{" "}
              {user.BusinessProfile?.endDay} ({user.BusinessProfile?.startTime}{" "}
              - {user.BusinessProfile?.endTime})
            </p>
          </div>
        </div>
      ) : (
